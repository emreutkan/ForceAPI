...
class WorkoutProgram(TimestampedModel):
    """
    Represents a user's workout split program (e.g., Push/Pull/Legs, Upper/Lower).
    The cycle repeats round-robin after cycle_length days.
    """
    user = models.ForeignKey(CustomUser, on_delete=models.CASCADE, related_name='workout_programs')
    name = models.CharField(max_length=255)
    cycle_length = models.PositiveIntegerField()  # Total days in one cycle (e.g., 4 for PPL+Rest)
    is_active = models.BooleanField(default=False)
    activated_at = models.DateTimeField(null=True, blank=True)  # When the program was last activated

    class Meta:
        ordering = ['-created_at']

    def __str__(self):
        return f"{self.user.email} - {self.name} ({self.cycle_length}-day cycle)"

    def activate(self):
        """Set this program as active, deactivating all other programs for this user."""
        WorkoutProgram.objects.filter(user=self.user, is_active=True).update(is_active=False)
        self.is_active = True
        self.activated_at = timezone.now()
        self.save(update_fields=['is_active', 'activated_at'])


class WorkoutProgramDay(TimestampedModel):
    """
    Represents a single day slot in the workout program cycle.
    day_number is 1-indexed and must be unique within the program.
    """
    program = models.ForeignKey(WorkoutProgram, on_delete=models.CASCADE, related_name='days')
    day_number = models.PositiveIntegerField()  # 1-indexed position in the cycle
    name = models.CharField(max_length=255)  # e.g., "Push Day", "Pull Day", "Rest"
...
class WorkoutProgramExercise(TimestampedModel):
    """
    Represents an exercise planned for a specific day in the program,
    including how many sets the user intends to perform.
    """
    program_day = models.ForeignKey(WorkoutProgramDay, on_delete=models.CASCADE, related_name='exercises')
    exercise = models.ForeignKey(Exercise, on_delete=models.CASCADE)
    order = models.PositiveIntegerField(default=0)
    ...
    def __str__(self):
        return f"{self.program_day} - {self.exercise.name} x{self.target_sets} sets"
